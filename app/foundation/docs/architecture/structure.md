...
- `flows` — the flows in `flows/` a run may follow, the rules each keeps, the order runs took before flows, and how a run takes a flow's steps: each work, the review that judges it and the operator's step after them.
...
- **A flow's name is its file's, and never a path.** `flows.load` reads only a file directly in `flows/`, and one it cannot read or parse is refused like one that breaks a rule.
