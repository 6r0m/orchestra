...
- `workflow` — the run: `plan → assess → build → verify`, the stops and their named answers, the final gate, what a Stop does to it, and the `status` query — which also says what the run is doing now and since when.
...
- **A Stop is Temporal's cancellation, heard wherever the run waits (D31).** It ends the run `STOPPED` and runs no git; a git side effect already running lands first and decides how the run ends; the cleanup waits a bounded time, so a host whose worker is gone never holds it.
