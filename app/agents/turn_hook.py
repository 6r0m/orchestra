"""Append one agent event to a turn's events file. Called by the agents themselves, never by us.
    ...
    python app/agents/turn_hook.py <events-file> UserPromptSubmit|Stop|StopFailure  (Claude: payload on stdin)
    python app/agents/turn_hook.py <events-file> notify <payload>                   (Codex: payload as argument)
