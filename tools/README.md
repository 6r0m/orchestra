...
| file | what it is for |
|---|---|
| [demo.py](demo.py) | `make demo`: watch runs in a Workbench of its own, with fake agents that take a few watchable seconds a turn and call no model, every answer and control pressed as the operator presses it — its own worker started from its Workbench's stack panel, in a systemd scope of its own, and every run started by typing its task into the page's form, on the demo's own workflow queue; the engineer's terminal read as the page shows it, live, and each confirmation's question checked for what it must say; one run the whole way, the architect sending the plan back once, approved and merged, then offered neither Stop run nor Force terminate; one stopped while its engineer works and one while it waits — sent back first with a note typed into the page, which reaches the engineer's next plan — each keeping its worktree and branch; one force-terminated when its Stop cannot finish, its merge held in a git hook, which once let go still lands — termination cannot stop what a host is already doing; the worker stopped while an engineer works, its stages' settings gone with it, the run blocked by it until the run's own button starts it again and the failed stage is continued; the worker restarted while a run waits, its engineer's terminal open on its record and never drawn again past the page's old retry until the engineer's next turn reaches it, and the run then discarded; what the run stopped at its approval kept, removed, through a workflow of its own whose record then goes from Temporal too. A stack of its own beside the live one — its own workflow and target queues, polled only by its own worker — sharing only Temporal, and removed afterwards, its runs deleted from Temporal too. It needs Temporal up (`make up`), proves the workflow code this checkout holds, and prints the URL to watch |
| [demo_press.py](demo_press.py) · [demo_press.ps1](demo_press.ps1) | the demo's Windows side: press one of a run's buttons, or one in the stack's popover, in the Workbench in headless Edge, over the DevTools protocol, answer the page's own confirmation, and report what the page said — or that a run's view offers none of some buttons, or that a role's terminal, open on its record, was never drawn again until its next turn — WSL cannot reach Windows' loopback, and Windows reaches the Workbench's |
...
this checkout's `.env`. They are therefore **not hermetic** and deliberately not
...
is named after the process that made it, and those whose process is gone are
removed — never one a stage still running uses — as soon as the stack stops a
worker and has proven it gone, and by every worker as it starts, which covers a
