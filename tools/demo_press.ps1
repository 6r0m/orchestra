...
#   powershell -File tools/demo_press.ps1 <workbench url> <run id> <button label> <what the page should say>
# Runs tools/demo_press.py in this checkout's uv-managed Windows environment (app\foundation\envpath.py).
param([string] $Url, [string] $Run, [string] $Label, [string] $Said)
...
& $uv --project $repo run --locked --quiet python (Join-Path $repo "tools\demo_press.py") $Url $Run $Label $Said
exit $LASTEXITCODE
