"""The operator's workbench: one localhost page over every run. `python workbench.py`

It serves `workbench/` and a small JSON API over `client.py` on `http://127.0.0.1:<workbench_port>`:
the runs, a run's status and timeline, its change, starting a run and answering its stop. The
page opens each run's agent terminals directly on the worker of the run's host (`terminal.py`).
